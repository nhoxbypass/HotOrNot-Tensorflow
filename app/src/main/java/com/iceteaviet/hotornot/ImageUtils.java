...
public class ImageUtils {
    ...
    public static Bitmap getCroppedBitmap(Bitmap photoBitmap) {
        int x = 0, y = 0;

        if (photoBitmap.getWidth() > Constants.IMAGE_SIZE) {
            x = (photoBitmap.getWidth() - Constants.IMAGE_SIZE) / 2;
        }
        if (photoBitmap.getHeight() > Constants.IMAGE_SIZE) {
            y = (photoBitmap.getHeight() - Constants.IMAGE_SIZE) / 2;
        }

        return Bitmap.createBitmap(photoBitmap, x, y, Constants.IMAGE_SIZE, Constants.IMAGE_SIZE);
    }
}